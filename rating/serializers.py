...
class CustomerRatingSerializer(serializers.HyperlinkedModelSerializer):
    ...
    def validate(self, request, pk): #  Aquí habia un pk
        ...
        Assertions.assert_true_raise403(user_type == 'Customer', {'error': translate(language, "ERROR_NOT_A_CUSTOMER")})
        ...
        try:
            ...
            Assertions.assert_true_raise400(portfolio.isHidden is False, {'error': translate(language, "ERROR_NO_USER")})

            Assertions.assert_true_raise401(request.data['score'], {'error': translate(language, "ERROR_NO_SCORE")})

            Assertions.assert_true_raise401(isinstance(request.data['score'], int), {'error': translate(language, "ERROR_DECIMAL_SCORE")})
            Assertions.assert_true_raise401(request.data['score'] >= 1 and request.data['score'] <= 5,
                                            {'error': translate(language, "ERROR_RATING_OUT_OF_RANGE")})
            ...
            Assertions.assert_true_raise400(False, {'error': translate(language, "ERROR_OFFER_NOT_FOUND")})
