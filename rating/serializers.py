...
class CustomerRatingSerializer(serializers.HyperlinkedModelSerializer):
    ...
    def _service_create(json: dict, rating: Rating):
        ...
        Assertions.assert_true_raise401(rating.score >= 1 or rating.score <= 5, {'error': 'The rating cannot be less than 1 or more than 5 points.'})
    ...
    def validate(self, request, pk): #  Aquí habia un pk
        ...
        Assertions.assert_true_raise403(user is not None, {'error': 'You mustbe logged in to access this page.'})
        ...
        try:
            ...
            Assertions.assert_true_raise403(user.user_id == offer.eventLocation.customer.user.id,  {'error': 'You are not the owner of this offer.'})

            #       Ahora se mira si la oferta está en estado PAYMENT_MADE

            Assertions.assert_true_raise401(offer.status == 'PAYMENT_MADE', {'error': 'This offer is not ready to receive a rating yet.'})

            #       Finalmente, se mira si ha votado ya o no

            Assertions.assert_true_raise401(offer.rating is None, {'error':'This offer has already be rated. You cannot rate it twice.'})
