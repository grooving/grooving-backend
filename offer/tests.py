...
def template_create_offer(*args, codeResponse):
    ...
    try:
        ...
        request_offer = factory.post('/offer/')
        force_authenticate(request_offer, user=user, token=user.auth_token)

        response_offer = views.CreateOffer.post(request=request_offer)

        APITestCase.assertEqual(codeResponse, response_offer.status_code)

    except Exception as e:
        APITestCase.assertEqual(codeResponse, e.status_code)
